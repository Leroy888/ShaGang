...
clsSettings::clsSettings(QString path, QObject *parent) :
    QObject(parent)
{
    m_strPath = path;
    configIniWrite = new QSettings(m_strPath, QSettings::IniFormat);
//    configIniWrite->setIniCodec(QTextCodec::codecForName(codec.toLatin1()));
...
{
    fileExit=QFile::exists(m_strPath);
    return fileExit;
}
...
{
    QString settingsFilePath /*= QString(m_strPath)*/;
